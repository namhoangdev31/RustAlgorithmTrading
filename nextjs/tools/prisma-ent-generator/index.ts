...
generatorHandler({
  ...
  async onGenerate(options) {
    ...
    for (const modelName of overrides.softDeleteModels) {
      ...
      if (!model.fields.some((field) => field.name === "deletedAt")) throw new Error(`soft-delete model ${modelName} has no deletedAt field`);
    }
    for (const key of Object.keys(overrides.typedJson)) {
      ...
      if (rest.length > 0 || !model || !field || field.type !== "Json") throw new Error(`typed JSON override references unknown JSON field ${key}`);
    }
