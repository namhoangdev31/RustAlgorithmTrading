...
export function ActivityTab({ data, locale, formatRelativeTime }: { data: ProjectTabData; locale: string; formatRelativeTime: (v: any, l: string) => string }) {
  ...
  return (
    <div className="rounded-lg border border-hairline bg-canvas overflow-hidden">
      <div className="px-5 py-4 border-b border-hairline-cool bg-canvas-soft/60">
        <h3 className="text-ink text-base font-semibold">Recent Activity</h3>
        <p className="text-ink-mute text-xs mt-1">Deployment, updates, and status changes from all projects.</p>
      </div>
          ...
          <div key={event.key} className="px-5 py-4 flex items-start justify-between gap-4">
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                ...
                <span className={`text-[11px] px-2 py-0.5 rounded-full border ${event.tone}`}>{formatRelativeTime(event.time, locale)}</span>
              </div>
