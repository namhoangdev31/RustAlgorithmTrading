...
export default async function BillingPage() {
  ...
  if (data.providerUnavailable) {
    return (
      <div className="space-y-6">
        <PageHeader title="Partner Payouts & Billing" description={`Stripe Connect billing for ${organization.name}.`} />
        <UnavailableState
          title="Stripe Connect unavailable"
          description="Billing and payout data cannot be loaded until a server-side Stripe credential is configured. No substitute balances or payouts are generated."
      ...
      {!data.connected ? (
        ...
        <Card className="border-emerald-500/20 bg-emerald-500/5">
          <CardHeader>
            ...
            <CardDescription className="text-emerald-700/80 dark:text-emerald-400/80 text-xs mt-1">
          ...
          <CardContent>
            <form action={onboardPartnerAction} className="flex items-center gap-4">
              ...
              <Button type="submit" size="sm" className="bg-emerald-600 hover:bg-emerald-700 text-white font-medium flex items-center gap-2 h-9 text-xs">
        ...
        <>
          <div className="flex items-center justify-between bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-4 text-xs">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-semibold text-emerald-950 dark:text-emerald-100 text-sm">Stripe Connect Account Active</h4>
                <p className="text-[10px] text-emerald-800/80 dark:text-emerald-300/80 font-mono mt-0.5">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
            <Card className="bg-gradient-to-br from-emerald-500/10 to-teal-500/5 border-emerald-500/20">
              ...
              <CardContent>
                ...
                <p className="text-[10px] text-muted-foreground mt-1">Ready for automatic payout</p>
              </CardContent>
            </Card>

            <Card className="bg-card border border-hairline">
              <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
                <CardTitle className="text-sm font-semibold">Pending Balance</CardTitle>
              ...
              <CardContent>
                ...
                <p className="text-[10px] text-muted-foreground mt-1 font-sans">Clearing through bank pipeline</p>
              </CardContent>
            </Card>

            <Card className="bg-card border border-hairline">
              <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
                <CardTitle className="text-sm font-semibold">Net Payouts (70%)</CardTitle>
                <TrendingUp className="h-4 w-4 text-emerald-500" />
              </CardHeader>
              <CardContent>
                <div className="text-xl font-bold text-emerald-600 dark:text-emerald-400">{formatVnd(totalNet)}</div>
                <p className="text-[10px] text-muted-foreground mt-1">Total revenue paid out to partner</p>
              </CardContent>
            </Card>

            <Card className="bg-card border border-hairline">
              <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
                <CardTitle className="text-sm font-semibold">Platform Fees (30%)</CardTitle>
                <TrendingUp className="h-4 w-4 text-red-500" />
              </CardHeader>
              <CardContent>
                <div className="text-xl font-bold text-red-500">{formatVnd(platformFee)}</div>
                <p className="text-[10px] text-muted-foreground mt-1">Total platform commission share</p>
              </CardContent>
            </Card>

            <Card className="bg-card border border-hairline">
              <CardHeader className="flex flex-row items-center justify-between pb-2 space-y-0">
                <CardTitle className="text-sm font-semibold">Installs</CardTitle>
                <Package2 className="h-4 w-4 text-indigo-500" />
              </CardHeader>
              <CardContent>
                <div className="text-xl font-bold">{installMetrics.installs}</div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            ...
            <Card className="bg-card border border-hairline">
              <CardHeader>
                ...
                <CardDescription className="text-xs text-muted-foreground mt-1">Recently recorded marketplace plugin purchases.</CardDescription>
              </CardHeader>
              <CardContent>
                    ...
                    <table className="w-full caption-bottom text-xs">
                      ...
                      <tbody>
                        {data.transactions.map((tr) => (
                          <tr key={tr.id} className="border-b border-hairline transition-colors hover:bg-secondary/30">
                            <td className="p-4 align-middle font-mono text-[10px] truncate max-w-[120px]">{tr.buyerUserId}</td>
                            <td className="p-4 align-middle text-center font-semibold">{formatVnd(tr.amount)}</td>
                            <td className="p-4 align-middle text-right text-emerald-600 dark:text-emerald-400 font-semibold">
            ...
            <Card className="bg-card border border-hairline">
              <CardHeader>
                ...
                <CardDescription className="text-xs text-muted-foreground mt-1">Clearing events from Stripe to your bank account.</CardDescription>
              </CardHeader>
              <CardContent>
                {data.payouts.length === 0 ? (
                  ...
                  <div className="relative w-full overflow-auto">
                    <table className="w-full caption-bottom text-xs">
                      ...
                      <tbody>
                        {data.payouts.map((po) => (
                          <tr key={po.id} className="border-b border-hairline transition-colors hover:bg-secondary/30">
                            <td className="p-4 align-middle max-w-[150px] truncate" title={po.bankName}>
                              {po.bankName}
                            </td>
                            <td className="p-4 align-middle text-center font-semibold">{formatVnd(po.amount)}</td>
