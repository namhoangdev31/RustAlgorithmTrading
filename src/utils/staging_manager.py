...
class StagingHardeningManager:
    ...
    def build_record(
        ...
        if "KILL_SWITCH" in scenario_id.upper() and (latency_sec is None or record_timestamp is None):
            disposition = "BLOCKED"
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        kill_switch_latencies = [r.latency_sec for r in self.records if r.latency_sec is not None and "KILL_SWITCH" in r.scenario_id.upper()]
        rollback_results = [r.rollback_success for r in self.records if r.rollback_success is not None and "ROLLBACK" in r.scenario_id.upper()]
        
        max_kill_switch_latency = max(kill_switch_latencies) if kill_switch_latencies else 0.0
        rollback_success_rate = sum(1 for r in rollback_results if r) / len(rollback_results) if rollback_results else 1.0
        alert_records = [r for r in self.records if "ALERT" in r.scenario_id.upper()]
        alert_fp_rate = max((float(r.metadata.get("fp_rate", 0.0)) for r in alert_records), default=0.0)
        critical_fn_count = sum(int(r.metadata.get("fn_count", 0)) for r in alert_records)
        throughput_watermark = max((int(r.metadata.get("throughput", 0)) for r in self.records), default=0)
