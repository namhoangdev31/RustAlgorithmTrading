        ...
        for i in range(50, len(data)):  # Start after enough data for indicators
            ...
            if symbol in self.active_positions:
                exit_signal = self._check_exit_conditions(
                    symbol, i, current, previous, data, regime_stop_loss, bar_regime_config
                )
                if exit_signal:
            ...
            can_entry = (
                ...
                and bar_regime_config
                and bar_regime_config.get("enabled", False)
            )

            if can_entry:
                entry_signal = self._check_entry_conditions(
                    symbol, i, current, previous, data, bar_regime_config, current_regime
                )
                if entry_signal:
                    signals.append(entry_signal)

        exit_count = sum(1 for s in signals if s.signal_type == SignalType.EXIT)
        logger.info(f"📈 Generated {len(signals)} signals ({exit_count} exits)")
        return signals
    ...
    def _check_entry_conditions(
        ...
        if direction in ["long_only", "both"]:
            if (
                ...
                and not pd.isna(current["sma_50"])
            ):
        ...
        conf_score = rsi_strength * 0.4 + macd_strength * 0.3 + volume_strength * 0.3
