...
final class PermissionManager {
    ...
    private func getShellDecision(appId: String, permission: Permission) async -> ShellDecision {
        if let isGranted = await DatabaseService.shared.getPermission(appId: appId, permission: permission.rawValue) {
            return isGranted ? .approved : .denied
        } else {
            return .notAsked
        }
    }
    ...
    private func saveShellDecision(appId: String, permission: Permission, decision: ShellDecision) async {
        ...
        await DatabaseService.shared.savePermission(appId: appId, permission: permission.rawValue, granted: granted)
    }
