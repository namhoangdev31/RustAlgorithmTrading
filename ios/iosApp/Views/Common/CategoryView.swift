...
struct CategoryBrowseView: View {
    ...
    let categories: [BrowseCategoryItem] = [
        ...
        BrowseCategoryItem(
            name: "Productivity", iconName: "list.bullet.clipboard.fill", color: .blue),
        ...
        BrowseCategoryItem(
            name: "Utilities", iconName: "wrench.and.screwdriver.fill", color: .gray),
        ...
        UniScrollView {
            LazyVGrid(columns: columns, spacing: 16) {
                ForEach(categories) { category in
                    UniButton(action: {
                        navigation.navigate(
                            to: .categoryDetail(id: category.id.uuidString, title: category.name))
                    }) {
                        VStack(spacing: 12) {
                            Circle()
