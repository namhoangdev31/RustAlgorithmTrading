...
def test_volume_filter_impact():
    ...
    for symbol in symbols:
        ...
        # Fetch data
        try:
            data = client.get_bars(symbol=symbol, timeframe="1Day", start=start_date, end=end_date)
        ...
        strategy_old = MomentumStrategy(
            volume_confirmation=True, volume_multiplier=1.2, macd_histogram_threshold=0.0005
        )
        ...
        strategy_new = MomentumStrategy(
            volume_confirmation=True, volume_multiplier=1.05, macd_histogram_threshold=0.0005
        )
        ...
        increase_pct = (
            (increase / len(entry_signals_old) * 100) if len(entry_signals_old) > 0 else 0
        )
