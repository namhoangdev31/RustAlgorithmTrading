...
def analyze_exit_patterns(results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not trades:
        return {"error": "No trades found in backtest results", "total_trades": 0}
...
def print_verification_report(analysis: Dict[str, Any], result_file: str):
    ...
    if sl_count > 0:
        ...
        print(
            f"   ⏳ DELAYED EXITS (>= 10 bars): {sl_stats['enforced_holding_period']} "
        ...
        if sl_stats["bypassed_pct"] > 50:
            print(
                f"   ✅ PASS: {sl_stats['bypassed_pct']:.1f}% of stop-losses bypass holding period"
            )
        else:
            print(
                f"   ⚠️ WARNING: Only {sl_stats['bypassed_pct']:.1f}% bypass holding period "
                f"(expected >50%)"
            )
    else:
    ...
    if tp_count > 0:
        ...
        print(
            f"   ⏳ DELAYED EXITS (>= 10 bars): {tp_stats['enforced_holding_period']} "
