...
def run_week35_validation():
    ...
    logger.info(
        f"  Mean Reversion Enabled: {strategy.parameters.get('mean_reversion_enabled', False)}"
    )
    ...
    if results and results.get("total_signals", 0) > 0:
        ...
        week2_metrics = {"win_rate": 0.333, "sharpe": 0.015, "trades": 69, "return": 0.0421}

        week3_metrics = {"win_rate": 0.267, "sharpe": -0.378, "trades": 15, "return": -0.257}

        logger.info("                 Week 2    Week 3    Week 3.5   Target")
        logger.info("-" * 80)
        logger.info(
            f"Win Rate:        {week2_metrics['win_rate']:.1%}     {week3_metrics['win_rate']:.1%}     {win_rate:.1%}      38-42%"
        )
        logger.info(
            f"Sharpe Ratio:    {week2_metrics['sharpe']:.3f}    {week3_metrics['sharpe']:.3f}    {sharpe_ratio:.3f}      >0.3"
        )
        logger.info(
            f"Total Trades:    {week2_metrics['trades']}       {week3_metrics['trades']}        {total_trades}         35-50"
        )
        logger.info(
            f"Total Return:    {week2_metrics['return']:.2%}    {week3_metrics['return']:.1%}    {total_return:.2%}     >+2%"
        )
        logger.info("")
        ...
        if 0.38 <= win_rate <= 0.42:
            ...
            logger.warning(
                f"✗ Win Rate: {win_rate:.1%} - Below target (need {0.38 - win_rate:.1%} more)"
            )
        ...
        if sharpe_ratio >= 0.3:
            ...
            logger.warning(
                f"✗ Sharpe Ratio: {sharpe_ratio:.2f} - Below target (need {0.3 - sharpe_ratio:.2f} more)"
            )
        ...
        if 35 <= total_trades <= 50:
            ...
            logger.warning(
                f"✗ Total Trades: {total_trades} - Below target (need {35 - total_trades} more)"
            )
        else:
            logger.warning(
                f"✗ Total Trades: {total_trades} - Above target ({total_trades - 50} over)"
            )
        ...
        if total_return >= 0.02:
            ...
            logger.warning(
                f"✗ Total Return: {total_return:.2%} - Below target (need {0.02 - total_return:.2%} more)"
            )
        ...
        results_output = {
            ...
            "comparison": {"week2": week2_metrics, "week3": week3_metrics},
