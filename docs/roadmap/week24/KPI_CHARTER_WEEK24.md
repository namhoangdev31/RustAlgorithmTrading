...
| Controlled live ready gate | `100%` | precondition + regression + rollback + blocker + approval | PASS: all criteria satisfied by command evidence | `CAPTURED_PASS` | `EV-W24-202` | `planner` |
...
| Release rerun stability | no new blocker | rerun pass | PASS: W21/W22 guards pass by command | `CAPTURED_PASS` | `EV-W24-207` | `tester` |
| Budget governance | files<=15 or approved escalation, LOC<=700 | approved escalation; net LOC within threshold | PASS | `CAPTURED_PASS` | `EV-W24-209` | `planner` |
| W09-W23 regression guard | `100%` | required slices pass | PASS: W21/W22/W23 guards pass by command | `CAPTURED_PASS` | `EV-W24-301..306` | `tester` |
