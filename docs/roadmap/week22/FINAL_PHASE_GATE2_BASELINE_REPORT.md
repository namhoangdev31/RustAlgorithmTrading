...
- Baseline mode: `EXECUTED` (`GO track` do W21 đã đạt GO precondition).
...
| `EV-W22-207` | Hard-gate rerun stability | no new blocker after rerun | PASS: all mandatory blockers closed | `CAPTURED_PASS` | `W22-ISS-003` |
| `EV-W22-208` | Release blocker mapping audit | blockers taxonomy complete | PASS: blockers mapped | `CAPTURED_PASS` | `W22-ISS-005` |
| `EV-W22-209` | Escalation record integrity | trigger/owner/mitigation captured | PASS: within budget | `CAPTURED_PASS` | `W22-ISS-010` |
| `EV-W22-210` | Throughput/toil watermark | gate toil measured | PASS: throughput captured | `CAPTURED_PASS` | `W22-ISS-011` |
...
| `EV-W22-305` | W17-W18 staging/canary guard | no regression | PASS (`verify_w15_capital_allocation.py`) | `CAPTURED_PASS` |
| `EV-W22-306` | W19-W21 safety/gate1 guard | no regression | PASS (all guard verifiers pass) | `CAPTURED_PASS` |
| `EV-W22-401` | Baseline -> Issue consistency | all blockers mapped | PASS: synchronized | `CAPTURED_PASS` |
| `EV-W22-402` | Artifact consistency | one final verdict | PASS: verdict duy nhất `GO` | `CAPTURED_PASS` |
