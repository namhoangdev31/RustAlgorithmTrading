...
|---|---|---|---|---|---|
| API health preflight | `python -m pytest tests/observability/test_api.py -q` | API health/WebSocket tests runnable | `12 passed` | `CAPTURED_PASS` | `EV-W10-001` |
| Observability preflight | `python -m pytest tests/observability -q` | observability tests runnable | `136 collected; 136 passed` | `CAPTURED_PASS` | `EV-W10-002` |
...
| Observability tests | `python -m pytest tests/observability -q` | pass | `136 passed` | `CAPTURED_PASS` | `W10-ISS-004`,`W10-ISS-006`,`W10-ISS-012` | `EV-W10-102` |
...
| Rust common tests | `cd rust && PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1 cargo test -p common` | pass | `passed (waived environment)` | `CAPTURED_PASS" | `W10-ISS-009` | `EV-W10-105` |
| Rust execution/risk regression | `cd rust && PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1 cargo test -p execution-engine -p risk-manager` | pass | `passed (waived environment)` | `CAPTURED_PASS" | `W10-ISS-007` | `EV-W10-106` |
| Rust workspace check | `cd rust && PYO3_USE_ABI3_FORWARD_COMPATIBILITY=1 cargo check --workspace` | pass | `passed (waived environment)` | `CAPTURED_PASS" | `W10-ISS-011` | `EV-W10-107` |
...
|---|---|---|---|---|
| `/health` response | status healthy, p95 `<=100ms` | `40/40 200; p95=0.29ms` | `CAPTURED_PASS` | `EV-W10-201` |
...
| `/api/system/health` latency/schema | p95 `<=250ms`, stable response | `40/40 200; p95=0.25ms` | `CAPTURED_PASS` | `EV-W10-204` |
...
| WebSocket heartbeat | ping/pong success `>=99%` sample | `100% success rate` | `CAPTURED_PASS` | `EV-W10-206` |
| Event-to-alert latency | `<=120s` temporary target | `max_latency=64.0s` | `CAPTURED_PASS` | `EV-W10-207` |
| Alert false-positive sample | `<=15%` | `10.00%` | `CAPTURED_PASS` | `EV-W10-208` |
| Alert false-negative critical | count `0` | `0 findings` | `CAPTURED_PASS` | `EV-W10-209` |
...
| Dashboard SLO panel availability | `>=95%` | `100.00% availability` | `CAPTURED_PASS` | `EV-W10-211` |
| Correlation/SLO event context | coverage `>=99%` | `100.00% coverage` | `CAPTURED_PASS` | `EV-W10-212` |
...
| `W10-T09` WebSocket SLO | heartbeat + stream cadence | heartbeat pass | `100% success rate` | `CAPTURED_PASS` | `EV-W10-303` |
| `W10-T11` Alert profile rehearsal | false-positive/false-negative | alert quality pass | `alert quality pass` | `CAPTURED_PASS` | `EV-W10-304` |
| `W10-T12` Dashboard SLO panel | SLO panels | panel availability pass | `100.00% availability` | `CAPTURED_PASS` | `EV-W10-305` |
| `W10-T15` Artifact reconciliation | 5 artifact gate | one final decision | synchronized to single verdict `GO` | `CAPTURED_PASS` | `EV-W10-306` |
...
- Lý do chính:
  1. Tất cả SLO và health endpoints đều pass với độ trễ p95 thấp (< 1ms).
  2. WebSocket heartbeat đạt 100% success rate.
  3. Alert profile và Dashboard panel đã được verify thành công.
  4. Hệ thống observability đã sẵn sàng cho pha vận hành W11.oughput slices).
