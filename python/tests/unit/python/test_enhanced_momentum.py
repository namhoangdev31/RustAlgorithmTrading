...
def strategy():
    ...
    return EnhancedMomentumStrategy(symbols=["TEST"], min_signal_quality=SignalQuality.MODERATE)
...
def aggressive_strategy():
    ...
    return EnhancedMomentumStrategy(
        symbols=["TEST"], risk_params=risk_params, min_signal_quality=SignalQuality.WEAK
    )
...
class TestIndicatorCalculations:
    ...
    def test_calculate_indicators(self, strategy, sample_ohlcv_data):
        ...
        assert (valid_rsi >= 0).all() and (valid_rsi <= 100).all(), "RSI values out of bounds"
    ...
    def test_macd_calculation(self, strategy, sample_ohlcv_data):
        ...
        np.testing.assert_array_almost_equal(
            data.loc[valid_idx, "macd_histogram"].values, calculated_histogram.values, decimal=5
        )
...
class TestSignalGeneration:
    ...
    def test_signal_quality_filtering(self, sample_ohlcv_data):
        ...
        strict_strategy = EnhancedMomentumStrategy(
            symbols=["TEST"], min_signal_quality=SignalQuality.STRONG
        )

        # Strategy accepting WEAK signals
        lenient_strategy = EnhancedMomentumStrategy(
            symbols=["TEST"], min_signal_quality=SignalQuality.WEAK
        )
        ...
        assert len(lenient_signals) >= len(
            strict_signals
    ...
    def test_volume_filter(self, sample_ohlcv_data):
        ...
        with_filter = EnhancedMomentumStrategy(
            symbols=["TEST"], enable_volume_filter=True, min_signal_quality=SignalQuality.WEAK
        )

        # Without volume filter
        without_filter = EnhancedMomentumStrategy(
            symbols=["TEST"], enable_volume_filter=False, min_signal_quality=SignalQuality.WEAK
        )
        ...
        assert len(signals_without) >= len(signals_with), "Removing filter should increase signals"

    def test_trend_filter(self, sample_ohlcv_data):
        """Test trend direction filter"""
        # With trend filter
        with_filter = EnhancedMomentumStrategy(
            symbols=["TEST"], enable_trend_filter=True, min_signal_quality=SignalQuality.WEAK
        )

        # Without trend filter
        without_filter = EnhancedMomentumStrategy(
            symbols=["TEST"], enable_trend_filter=False, min_signal_quality=SignalQuality.WEAK
        )
        ...
        # Without filter should generate more or equal signals
        assert len(signals_without) >= len(
            signals_with
...
class TestRiskManagement:
    ...
    def test_stop_loss_calculation(self, strategy, sample_ohlcv_data):
        ...
        stop_loss, take_profit, _ = strategy.calculate_risk_metrics(
            entry_price=row["close"], signal_type=SignalType.BUY, atr=row["atr"]
        )
        ...
        assert abs(actual_distance - expected_distance) < 0.01, "Stop loss distance incorrect"
    ...
    def test_risk_reward_ratio(self, strategy, sample_ohlcv_data):
        ...
        assert abs(risk_reward - expected_ratio) < 0.01, "Risk/reward ratio calculation incorrect"
...
class TestPositionSizing:
    ...
    def test_position_size_respects_max_limit(self, strategy):
        ...
        assert position_size <= max_shares, "Position size exceeds maximum allowed"
    ...
    def test_position_size_scales_with_confidence(self, strategy):
        ...
        scaling_strategy = EnhancedMomentumStrategy(
            symbols=["TEST"],
            risk_params=RiskParameters(
                max_position_size=1.0, risk_per_trade=0.02  # No limit  # Lower risk per trade
            ),
        )
...
class TestPerformanceTracking:
    ...
    def test_signal_counting(self, strategy, sample_ohlcv_data):
        ...
        assert strategy.total_signals_generated == initial_count + len(
            signals
    ...
    def test_quality_distribution_tracking(self, strategy, sample_ohlcv_data):
        ...
        assert (
            total_by_quality == strategy.total_signals_generated
