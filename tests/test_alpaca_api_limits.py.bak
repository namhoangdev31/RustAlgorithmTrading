...
class TestDateRangeCalculation(unittest.TestCase):
    ...
    def test_end_date_not_in_future(self):
        ...
        config = DownloadConfig(symbols=["AAPL"], start_date="2024-01-01", end_date=future_date)

        # End date should be capped at today
        end_date_parsed = datetime.strptime(config.end_date, "%Y-%m-%d").date()
        self.assertLessEqual(
            end_date_parsed, today, f"End date {end_date_parsed} should not be after today {today}"
        )

    def test_date_range_validation(self):
        """Validate start date is before end date"""
        config = DownloadConfig(symbols=["AAPL"], start_date="2024-12-31", end_date="2024-01-01")

        start = datetime.strptime(config.start_date, "%Y-%m-%d")
        end = datetime.strptime(config.end_date, "%Y-%m-%d")

        self.assertLess(start, end, "Start date should be before end date")

    def test_weekend_handling(self):
        """Test that weekends are handled properly"""
        # Friday to Monday should work
        config = DownloadConfig(
            symbols=["AAPL"], start_date="2024-01-05", end_date="2024-01-08"  # Friday  # Monday
        )
    ...
    def test_90_day_fallback_range(self):
        ...
        self.assertLessEqual(days_90_ago, today, "90-day range should not extend to future")

        # Verify it's exactly 90 days
        delta = (today - days_90_ago).days
        self.assertEqual(delta, 90, f"90-day range should be exactly 90 days, got {delta}")

    def test_date_format_validation(self):
        """Test date format is correct YYYY-MM-DD"""
        valid_dates = ["2024-01-01", "2024-12-31", "2025-06-15"]
...
class TestRetryLogic(unittest.TestCase):
    ...
    def test_retry_on_api_failure(self, mock_client_class):
        ...
        config = DownloadConfig(
            symbols=["AAPL"], start_date="2024-01-01", end_date="2024-01-31", retry_attempts=3
        )
    ...
    def test_no_data_returned_handling(self, mock_client_class):
        ...
        config = DownloadConfig(
            symbols=["AAPL"], start_date="2024-01-01", end_date="2024-01-31", retry_attempts=1
        )
...
class TestExponentialBackoff(unittest.TestCase):
    ...
    def test_backoff_timing_calculation(self):
        ...
        base_delay = 5
        expected_delays = [5, 10, 20, 40]  # 5 * 2^0  # 5 * 2^1  # 5 * 2^2  # 5 * 2^3
...
class TestDataValidation(unittest.TestCase):
    ...
    def test_valid_dataframe(self):
        ...
        config = DownloadConfig(symbols=["AAPL"], start_date="2024-01-01", end_date="2024-01-31")
