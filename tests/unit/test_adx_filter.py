...
class TestADXFilter:
    ...
    def test_adx_blocks_ranging_market_signals(self, strategy_with_adx, ranging_market_data):
        ...
        entry_signals_with = [
            s for s in signals_with_filter if s.signal_type in [SignalType.LONG, SignalType.SHORT]
        ]
        entry_signals_without = [
            s
            for s in signals_without_filter
        ...
        assert len(entry_signals_with) <= len(
            entry_signals_without
    ...
    def test_week3_expected_trade_reduction(self, strategy_with_adx, strategy_without_adx):
        ...
        trades_with = len(
            [s for s in signals_with if s.signal_type in [SignalType.LONG, SignalType.SHORT]]
        )
        trades_without = len(
            [s for s in signals_without if s.signal_type in [SignalType.LONG, SignalType.SHORT]]
        )
