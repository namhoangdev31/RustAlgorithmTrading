...
class LogDataGenerator:
    @staticmethod
    def generate_single_log(level="INFO", service="trading-engine", message="Test message", metadata=None, correlation_id=None) -> Dict[str, Any]:
        return {
    ...
    def generate_batch(count=100, correlation_id=None) -> List[Dict[str, Any]]:
        ...
        for i in range(count):
            ...
            logs.append(LogDataGenerator.generate_single_log(
    ...
    def generate_error_logs(error_type: str, severity: str) -> List[Dict[str, Any]]:
        ...
        logs = [
            LogDataGenerator.generate_single_log(level="WARNING", message="Retrying...", correlation_id=corr_id),
            LogDataGenerator.generate_single_log(level=severity, message=f"Failed with {error_type}", correlation_id=corr_id, metadata={"error": error_type}),
            LogDataGenerator.generate_single_log(level="INFO", message="Handled error", correlation_id=corr_id)
        ]
        return logs
