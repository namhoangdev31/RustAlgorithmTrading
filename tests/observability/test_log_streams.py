...
class TestMarketDataLogger:
    ...
    def test_price_update(self, logger, caplog):
        ...
        with caplog.at_level(logging.DEBUG):
            logger.log_price_update(symbol="BTCUSDT", price=45000.0, volume=1.5, source="binance")
    ...
    def test_orderbook_update(self, logger, caplog):
        ...
        with caplog.at_level(logging.DEBUG):
            logger.log_orderbook_update(
                symbol="ETHUSDT", bid_price=3000.0, ask_price=3001.0, bid_size=10.0, ask_size=8.0
            )
    ...
    def test_trade_logging(self, logger, caplog):
        ...
        with caplog.at_level(logging.INFO):
            logger.log_trade(symbol="BTCUSDT", price=45000.0, quantity=0.5, side="buy")
    ...
    def test_feed_status(self, logger, caplog):
        ...
        with caplog.at_level(logging.INFO):
            logger.log_feed_status(
                source="binance", status="connected", message="Successfully connected"
            )
    ...
    def test_data_quality_issue(self, logger, caplog):
        ...
        with caplog.at_level(logging.WARNING):
            logger.log_data_quality_issue(
                issue_type="stale_data", symbol="BTCUSDT", details="No updates for 10 seconds"
            )
    ...
    def test_position_update(self, logger, caplog):
        ...
        with caplog.at_level(logging.DEBUG):
            logger.log_position_update(
                strategy_name="momentum", symbol="BTCUSDT", position=0.5, pnl=250.0
            )
    ...
    def test_strategy_state(self, logger, caplog):
        ...
        with caplog.at_level(logging.INFO):
            logger.log_strategy_state(
                strategy_name="momentum", state="active", reason="Market conditions favorable"
            )
    ...
    def test_performance_metric(self, logger, caplog):
        ...
        with caplog.at_level(logging.INFO):
            logger.log_performance_metric(
                strategy_name="momentum", metric_name="sharpe_ratio", value=1.8, period="30d"
            )
    ...
    def test_exposure_update(self, logger, caplog):
        ...
        with caplog.at_level(logging.DEBUG):
            logger.log_exposure_update(
                exposure_type="total_notional", value=50000.0, limit=100000.0
            )
...
class TestExecutionLogger:
    ...
    def test_order_status(self, logger, caplog):
        ...
        with caplog.at_level(logging.INFO):
            logger.log_order_status(order_id="order-123", status="filled", filled_quantity=0.5)
    ...
    def test_execution_quality(self, logger, caplog):
        ...
        with caplog.at_level(logging.DEBUG):
            logger.log_execution_quality(order_id="order-123", metric_name="slippage", value=0.02)
...
class TestSystemLogger:
    ...
    def test_startup(self, logger, caplog):
        ...
        with caplog.at_level(logging.INFO):
            logger.log_startup(component="trading_engine", version="1.0.0")
    ...
    def test_shutdown(self, logger, caplog):
        ...
        with caplog.at_level(logging.INFO):
            logger.log_shutdown(component="trading_engine", reason="User requested")
    ...
    def test_health_check(self, logger, caplog):
        ...
        with caplog.at_level(logging.DEBUG):
            logger.log_health_check(
                component="database", status="healthy", details={"latency_ms": 5}
            )
    ...
    def test_resource_usage(self, logger, caplog):
        ...
        with caplog.at_level(logging.DEBUG):
            logger.log_resource_usage(
                resource_type="memory", value=512.0, unit="MB", threshold=1024.0
            )
    ...
    def test_config_change(self, logger, caplog):
        ...
        with caplog.at_level(logging.INFO):
            logger.log_config_change(
                config_key="max_position", old_value=1.0, new_value=2.0, changed_by="admin"
            )
