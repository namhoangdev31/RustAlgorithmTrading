...
import (
	...
	"trading/control-gateway/internal/modules/operations/domain"
)
...
func incidentView(value domain.Incident) incidentResponse {
	return incidentResponse{ID: value.ID, Title: value.Title, Severity: value.Severity, Status: value.Status, Component: value.Component, ReasonCode: value.ReasonCode, CorrelationID: value.CorrelationID, Owner: value.Owner, Evidence: value.Evidence, CreatedAt: value.CreatedAt, UpdatedAt: value.UpdatedAt, Metadata: value.Metadata}
}

// AlertHandler handles HTTP requests relating to alerts and incidents.
type AlertHandler struct {
	useCase application.AlertUseCase
}

// NewAlertHandler creates a new instance of AlertHandler.
func NewAlertHandler(useCase application.AlertUseCase) *AlertHandler {
