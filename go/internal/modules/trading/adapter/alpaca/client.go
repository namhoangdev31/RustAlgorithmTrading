...
func (c *Client) GetAccount(ctx context.Context) (application.AlpacaAccount, error) {
	var out alpacaAccountWire
	err := c.doTradingJSON(ctx, http.MethodGet, "/v2/account", nil, &out)
	return application.AlpacaAccount(out), err
}

func (c *Client) GetPositions(ctx context.Context) ([]application.AlpacaPosition, error) {
	var out []alpacaPositionWire
	err := c.doTradingJSON(ctx, http.MethodGet, "/v2/positions", nil, &out)
	result := make([]application.AlpacaPosition, len(out))
	for index := range out {
		result[index] = application.AlpacaPosition(out[index])
	}
	return result, err
}
...
func (c *Client) PlaceMarketOrder(ctx context.Context, symbol string, qty float64, side string, tif string) (application.OrderResponse, error) {
	if tif == "" {
		tif = "day"
	}
	req := orderRequestWire{
		Symbol:      symbol,
	...
	return application.OrderResponse(out), err
}

func (c *Client) PlaceLimitOrder(ctx context.Context, symbol string, qty float64, side string, limitPrice float64, tif string) (application.OrderResponse, error) {
	if tif == "" {
		tif = "day"
	}
	req := map[string]any{
