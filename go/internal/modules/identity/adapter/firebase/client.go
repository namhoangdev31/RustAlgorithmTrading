...
func (c *Client) AuthenticatePassword(ctx context.Context, email, password string) (application.FirebaseUser, error) {
	...
	if err := c.request(ctx, "accounts:signInWithPassword", map[string]any{"email": email, "password": password, "returnSecureToken": true}, &response); err != nil {
		return application.FirebaseUser{}, err
	}
...
func (c *Client) request(ctx context.Context, method string, body any, output any) error {
	if c.apiKey == "" {
		return apperror.WithMessage(apperror.ErrUnavailable, "firebase api key is not configured")
	}
	...
	if err != nil {
		return apperror.Wrap(apperror.CodeUnavailable, "firebase request failed", err)
	}
...
func (u firebaseUser) domain() application.FirebaseUser {
	...
	return application.FirebaseUser{LocalID: u.LocalID, Email: u.Email, DisplayName: u.DisplayName, PhotoURL: u.PhotoURL, ProviderID: provider}
}
