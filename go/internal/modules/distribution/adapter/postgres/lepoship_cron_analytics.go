...
func (r *Repository) runBundleAbuse(ctx context.Context) (application.CronJobResult, error) {
	now := time.Now().UTC()
	...
	if err != nil {
		return application.CronJobResult{}, fmt.Errorf("scan abuse reports: %w", err)
	}
	...
	if err != nil {
		return application.CronJobResult{}, fmt.Errorf("begin abuse transaction: %w", err)
	}
	rollback := true
	defer func() {
		if rollback {
	...
	for bundleID, reporters := range fingerprints {
		...
		if err := tx.BundleAbuseSignals.Create().SetID(uuid.New()).SetBundleId(bundleID).
			...
			return application.CronJobResult{}, fmt.Errorf("upsert abuse signal: %w", err)
		}
		...
		if err != nil {
			return application.CronJobResult{}, fmt.Errorf("transition bundle review state: %w", err)
		}
		if count > 0 {
			...
			if _, err := tx.BundleStateTransitions.Create().SetID(uuid.New()).SetBundleId(bundleID).
				...
				return application.CronJobResult{}, fmt.Errorf("record state transition: %w", err)
			}
		}
	}
	if err := tx.Commit(); err != nil {
		return application.CronJobResult{}, fmt.Errorf("commit abuse transaction: %w", err)
	}
	rollback = false
	return application.CronJobResult{Job: "bundle-abuse", Processed: processed, Skipped: transitioned, Message: "flagged abusive bundles and transitioned published bundles"}, nil
}

func (r *Repository) runRetentionCalculator(ctx context.Context) (application.CronJobResult, error) {
	now := time.Now().UTC()
	...
	if err != nil {
		return application.CronJobResult{}, fmt.Errorf("list bundles: %w", err)
	}
	...
	if err != nil {
		return application.CronJobResult{}, fmt.Errorf("load retention analytics: %w", err)
	}
	...
	if err != nil {
		return application.CronJobResult{}, fmt.Errorf("load retention installs: %w", err)
	}
	reviews, err := r.client.BundleReviews.Query().All(ctx)
	if err != nil {
		return application.CronJobResult{}, fmt.Errorf("load bundle reviews: %w", err)
	}
	...
	if err := r.client.BundleInstallEvents.Query().
		...
		return application.CronJobResult{}, fmt.Errorf("aggregate install totals: %w", err)
	}
	...
	for _, bundleID := range bundleIDs {
		...
		if err := r.client.BundleRetentionStats.Create().SetID(uuid.New()).SetBundleId(bundleID).
			...
			return application.CronJobResult{}, fmt.Errorf("upsert retention stats: %w", err)
		}
	...
	return application.CronJobResult{Job: "retention-calculator", Processed: int64(len(bundleIDs)), Message: "computed retention and refreshed bundle stats"}, nil
}

func (r *Repository) runRankingCalculator(ctx context.Context) (application.CronJobResult, error) {
	now := time.Now().UTC()
	rows, err := r.client.Bundles.Query().Where(bundles.StatusEQ(entschema.BundleCatalogStatusPublished)).
		...
		All(ctx)
	if err != nil {
		return application.CronJobResult{}, fmt.Errorf("load ranking candidates: %w", err)
	}
	...
	for index, bundle := range rows {
		...
		if err := r.client.BundleRankingScores.Create().SetID(uuid.New()).SetBundleId(score.ID).
			...
			return application.CronJobResult{}, fmt.Errorf("upsert ranking score: %w", err)
		}
	}
	...
	for _, category := range uniqueCategories(scores) {
		for index, score := range sortedByCategory(scores, category) {
			...
			if err := r.client.BundleTrendingSnapshots.Create().SetID(uuid.New()).SetBundleId(score.ID).
				...
				return application.CronJobResult{}, fmt.Errorf("upsert trending snapshot: %w", err)
			}
		}
	}
	return application.CronJobResult{Job: "ranking-calculator", Processed: int64(len(scores)), Message: "updated ranking scores and trending snapshots"}, nil
}
...
func (r *Repository) refreshBundleStatsFromRows(ctx context.Context, bundleID uuid.UUID, counts bundleInstallCounts, reviews []*entdb.BundleReviews, now time.Time) error {
	ratingCounts := [6]int{}
	var ratingTotal int
	for _, review := range reviews {
