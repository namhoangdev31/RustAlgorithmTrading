...
func (c *Client) GetAccount(ctx context.Context) (entities.AlpacaAccount, error) {
	var out entities.AlpacaAccount
	err := c.doTradingJSON(ctx, http.MethodGet, "/v2/account", nil, &out)
	return out, err
}

func (c *Client) GetPositions(ctx context.Context) ([]entities.AlpacaPosition, error) {
	var out []entities.AlpacaPosition
...
func (c *Client) PlaceMarketOrder(ctx context.Context, symbol string, qty float64, side string, tif string) (entities.OrderResponse, error) {
	if tif == "" {
		tif = "day"
	}
	...
	var out entities.OrderResponse
	err := c.doTradingJSON(ctx, http.MethodPost, "/v2/orders", req, &out)
	return out, err
}

func (c *Client) PlaceLimitOrder(ctx context.Context, symbol string, qty float64, side string, limitPrice float64, tif string) (entities.OrderResponse, error) {
	if tif == "" {
		tif = "day"
	}
	req := map[string]any{
