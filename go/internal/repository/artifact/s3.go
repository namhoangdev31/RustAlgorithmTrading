...
func NewS3Signer(ctx context.Context, input S3Config) (*S3Signer, error) {
	if strings.TrimSpace(input.AccessKeyID) == "" || strings.TrimSpace(input.SecretAccessKey) == "" {
		return nil, fmt.Errorf("%w: artifact credentials are not configured", repositories.ErrUnavailable)
	}
...
func (s *S3Signer) PresignDownload(ctx context.Context, artifact entities.BundleArtifact, expires time.Duration) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("%w: presign artifact download", repositories.ErrUnavailable)
	}
	if !strings.HasPrefix(request.URL, "http://") && !strings.HasPrefix(request.URL, "https://") {
		return "", fmt.Errorf("%w: signer returned a non-http url", repositories.ErrUnavailable)
	}
