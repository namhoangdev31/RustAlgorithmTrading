...
type riskLimitsUseCase struct {
	repo        repositories.RiskLimitsRepository
	redisClient *redis.Client
}

func NewRiskLimitsUseCase(repo repositories.RiskLimitsRepository, redisClient *redis.Client) usecases.RiskLimitsUseCase {
...
func (u *riskLimitsUseCase) GetRiskLimits(ctx context.Context, accountID *string) (*entities.RiskLimits, error) {
	return u.repo.GetRiskLimits(ctx, accountID)
}

func (u *riskLimitsUseCase) UpdateRiskLimits(ctx context.Context, limits *entities.RiskLimits, username string) error {
	if limits == nil {
	...
	if u.redisClient != nil {
		...
		if err != nil {
			...
			err = u.redisClient.Publish(ctx, channel, jsonBytes).Err()
			if err != nil {
