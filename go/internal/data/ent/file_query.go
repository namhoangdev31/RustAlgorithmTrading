...
// QueryUsersWithPhoto chains the current query on the "usersWithPhoto" edge.
func (_q *FileQuery) QueryUsersWithPhoto() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, file.UsersWithPhotoTable, file.UsersWithPhotoColumn),
		)
...
func (_q *FileQuery) Clone() *FileQuery {
	...
	return &FileQuery{
		...
		withUsersWithPhoto: _q.withUsersWithPhoto.Clone(),
...
// WithUsersWithPhoto tells the query-builder to eager-load the nodes that are connected to
// the "usersWithPhoto" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *FileQuery) WithUsersWithPhoto(opts ...func(*UserQuery)) *FileQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withUsersWithPhoto = query
	return _q
}
...
func (_q *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if query := _q.withUsersWithPhoto; query != nil {
		if err := _q.loadUsersWithPhoto(ctx, query, nodes,
			func(n *File) { n.Edges.UsersWithPhoto = []*User{} },
			func(n *File, e *User) { n.Edges.UsersWithPhoto = append(n.Edges.UsersWithPhoto, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *FileQuery) loadUsersWithPhoto(ctx context.Context, query *UserQuery, nodes []*File, init func(*File), assign func(*File, *User)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(user.FieldPhotoId)
	}
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(file.UsersWithPhotoColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.PhotoId
		if fk == nil {
			return fmt.Errorf(`foreign-key "photoId" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "photoId" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
