...
for config in "system.json" "system.staging.json" "system.production.json" "risk_limits.toml"; do
    if [ -f "$PROJECT_ROOT/ops/config/$config" ]; then
...
if [ -f "$PROJECT_ROOT/ops/config/risk_limits.toml" ]; then
    if grep -q "max_position_size" "$PROJECT_ROOT/ops/config/risk_limits.toml" && \
       grep -q "max_daily_loss" "$PROJECT_ROOT/ops/config/risk_limits.toml"; then
...
if [ -f "$PROJECT_ROOT/ops/deployment/Dockerfile" ] && [ -f "$PROJECT_ROOT/ops/deployment/docker-compose.yml" ]; then
...
if [ -f "$PROJECT_ROOT/ops/deployment/monitoring/prometheus.yml" ] && \
   [ -f "$PROJECT_ROOT/ops/deployment/monitoring/alertmanager.yml" ]; then
