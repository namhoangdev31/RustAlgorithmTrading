...
cd "$PROJECT_ROOT/go"
REDIS_URL=${REDIS_URL:-redis://127.0.0.1:6379} LEPOS_STORAGE_ROOT=${LEPOS_STORAGE_ROOT:-.} LEPOS_CONTROL_PLANE_URL=${LEPOS_CONTROL_PLANE_URL:-http://127.0.0.1:3000} LEPOS_INTERNAL_API_KEY=${LEPOS_INTERNAL_API_KEY:-lepos_dev_internal_key} LEPOS_SERVICE_ID=${LEPOS_SERVICE_ID:-edge-gateway} LEPOS_SERVICE_SECRET=${LEPOS_SERVICE_SECRET:-} LEPOS_IPFS_GATEWAY_URL=${LEPOS_IPFS_GATEWAY_URL:-https://ipfs.io/ipfs} LEPOS_ARWEAVE_GATEWAY_URL=${LEPOS_ARWEAVE_GATEWAY_URL:-https://arweave.net} LEPOS_CONTROL_PLANE_TLS_CERT=${LEPOS_CONTROL_PLANE_TLS_CERT:-} LEPOS_CONTROL_PLANE_TLS_KEY=${LEPOS_CONTROL_PLANE_TLS_KEY:-} LEPOS_CONTROL_PLANE_TLS_CA=${LEPOS_CONTROL_PLANE_TLS_CA:-} LEPOS_NATIVE_REMEDIATION_SCRIPT=${LEPOS_NATIVE_REMEDIATION_SCRIPT:-$PROJECT_ROOT/ops/scripts/native_remediation.sh} ./target/edge-gateway > "$LOG_DIR/edge-gateway.log" 2>&1 &
EDGE_GATEWAY_PID=$!
